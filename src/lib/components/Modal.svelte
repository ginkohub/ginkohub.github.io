	...
	>
		...
		>
			...
			<div class="flex items-center justify-between mb-6 border-b border-slate-800 pb-4">
				<h2 class="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 flex items-center gap-2">
					<span class="w-2 h-2 bg-red-500 animate-pulse"></span>
					{title}
				</h2>
				<span class="text-[8px] font-mono text-slate-600">ID: {Math.random().toString(16).slice(2, 8).toUpperCase()}</span>
			</div>
