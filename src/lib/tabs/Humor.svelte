...
<div class="space-y-10">
	...
	<div class="relative group/joke text-center md:text-left">
		<div class="flex justify-between items-center mb-4">
			<h2 class="text-[9px] font-bold uppercase tracking-widest text-slate-300">Random Logic</h2>
			<button 
				onclick={fetchJoke} 
				class="text-[8px] font-black uppercase border-b transition-all px-1 active:bg-slate-900 active:text-white"
		...
		<div class="space-y-3 min-h-[4rem]">
			<p class="text-lg md:text-xl font-bold leading-snug text-slate-800 font-space">{joke.setup}</p>
			{#if joke.punchline}
	...
	<!-- Meme Section -->
	<div class="py-10 border-t border-slate-50 relative group/meme">
		<div class="flex justify-between items-center mb-4">
			<h2 class="text-[9px] font-bold uppercase tracking-widest text-slate-300">Digital Artifact</h2>
			<button 
				onclick={fetchMeme} 
				class="text-[8px] font-black uppercase border-b transition-all px-1 active:bg-slate-900 active:text-white"
				style="border-color: var(--accent-color); color: var(--accent-color);"
		...
		<div class="space-y-3">
			{#if meme.url}
				<div class="relative bg-slate-50 overflow-hidden border border-slate-50 active:scale-[0.98] transition-transform duration-200">
					<img 
						src={meme.url} 
						alt={meme.title} 
			...
			{:else if meme.loading}
				<div class="h-40 bg-slate-50 animate-pulse"></div>
			{/if}
		</div>
	</div>
</div>
