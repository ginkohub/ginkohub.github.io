	...
	const intelligenceNodes = [
		{
			...
			description: 'The state-of-the-art multimodal model designed for real-time interaction and complex reasoning across text, audio, and vision.',
		...
		},
		{
			...
			description: 'Highly sophisticated reasoning capabilities with a focus on nuance, humor, and exceptional coding proficiency.',
		...
		},
		{
			...
			description: 'Revolutionary massive context window, capable of processing entire codebases or hours of video in a single prompt.',
...
<div class="space-y-6 pt-12 border-t border-slate-800/50" in:fade>
	...
	<div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
		...
		>
					...
					{#each Object.entries(selectedNode.benchmarks) as [key, value]}
						<div class="flex items-baseline gap-2">
							<span class="text-[8px] font-bold text-slate-600 uppercase tracking-widest"
								>{key.replace('_', ' ')}</span
							>
							<span class="text-2xl font-black text-white font-space italic" style="color: {accentColor}">
								{value}
							</span>
						</div>
					{/each}
			...
			<div class="mt-auto pt-6 flex justify-between items-center text-[7px] font-mono text-slate-700">
