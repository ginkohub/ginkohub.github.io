<script>
	...
	function createBricks() {
		...
		brickWidth = (width - (brickOffsetLeft * 2) - (brickPadding * (brickColumnCount - 1))) / brickColumnCount;
...
<div class="flex flex-col items-center space-y-6 w-full">
	...
	<div class="relative group border border-slate-800 bg-slate-900/50 w-full max-w-[600px]">
		...
		{#if !gameStarted}
			<div class="absolute inset-0 flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm transition-all">
				{#if gameOver}
					<span class="text-[10px] font-black text-rose-500 uppercase tracking-[0.3em] mb-2">Sector Lost</span>
					<span class="text-2xl font-bold text-white mb-6 font-space">GAME OVER</span>
				{:else}
					<span class="text-[10px] font-black text-blue-500 uppercase tracking-[0.3em] mb-2">Protocol: Breakout</span>
	...
	<div class="flex w-full max-w-[600px] gap-2 md:hidden pt-4 h-32">
		<button 
			ontouchstart={(e) => { e.preventDefault(); leftPressed = true; }} 
			ontouchend={(e) => { e.preventDefault(); leftPressed = false; }}
		...
		<button 
			ontouchstart={(e) => { e.preventDefault(); rightPressed = true; }} 
			ontouchend={(e) => { e.preventDefault(); rightPressed = false; }}
